...
# ======== 讀取檔案 -- 確認通道數用 ========
def channel_check(tile0, tile1, tile2, weight, show_detail = False):
    if(len(weight) != 4):
...
def DW(tile0, tile1, tile2, weight, stride, show_detail):
    '''
    Docstring for DW
    
    :param tile0: 二維陣列[ in_channel(0-3) ][ w(0-不一定) ]
    :param tile1: 二維陣列[ in_channel(0-3) ][ w(0-不一定) ]
    :param tile2: 二維陣列[ in_channel(0-3) ][ w(0-不一定) ]
    :param weight: 二維陣列[ out_channel(0-3) ][ b(0) + w(1-不一定) ]
    :param stride: 降採樣是 2，OG Shuffle 是 1
    ...
    if(show_detail): print("[完成]: DW 運算完成")
    return output
