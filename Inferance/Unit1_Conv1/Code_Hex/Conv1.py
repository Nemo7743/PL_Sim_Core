...
def transpose_list(input_matrix, show_detail = False):
    ...
    try:
        ...
        if show_detail:
            ...
            print(f"[系統]: 資料轉置完成！")
            print(f"       原始維度: {hex(rows)}x{hex(cols)} -> 新維度: {hex(new_rows)}x{hex(new_cols)}")
...
def file_to_list(file_path, show_detail=False):
    ...
    try:
        ...
        if show_detail:
            ...
            print(f"       讀入維度: {hex(rows)} 列 x  {hex(cols)} 行")
...
def Conv1_implementation():
    ...
    for i in range(0, 128, 2):
        ...
        output_filename = o_dst_root / f"row_{(i//2):03d}.txt"
        list_to_file(output_Hex, output_filename)
        print(f"[系統]: 第 {i//2} 個 Tile 運算完成並儲存至 {output_filename}")
        # 加入最終輸出的三維陣列
