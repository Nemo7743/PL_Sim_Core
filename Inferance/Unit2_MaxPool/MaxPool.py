...
def MaxPool_implementation():
    ...
    for i in range(0, 64, 2):
        ...
        for j in range(0, 24, 4):
            #print(j, j+1, j+2, j+3)
            # ========== 進行運算並把運算結果 concat ==========
            output_Dec_neet_transpose = output_Dec_neet_transpose + Max_Pool_Sim.MaxPool(fmap[i][j+0 : j+4], fmap[i+1][j+0 : j+4], fmap[i+2][j+0 : j+4], 2, False)
        ...
        # 儲存運算結果
        o_dst_root = Path(r"C:\Users\legoa\NCU\專題\專題內容\硬體模擬\PL_Sim_Core\Inferance\Unit2_MaxPool\Fmap_to_DownSampling0")
