...
def Splitting_Tile(input_file_path, output_file_path):
    ...
    for i in range(0, len(matrix[0]), 4):
        for j in range(0, len(matrix), 4):
            ...
            out_matrix.append(matrix[j+1][0+i : 4+i])
            out_matrix.append(matrix[j+2][0+i : 4+i])
            out_matrix.append(matrix[j+3][0+i : 4+i])
...
if __name__ == "__main__":
    ...
    input_file_path = r"C:\Users\legoa\NCU\專題\專題內容\硬體模擬\PL_Sim_Core\Calculations\PW_Sim\data\tile_buffer1.txt"
    output_file_path = r"C:\Users\legoa\NCU\專題\專題內容\硬體模擬\PL_Sim_Core\Calculations\PW_Sim\data\tile_buffer1_splitted.txt"
